...
module RedirectPatch
  def redirect_to(options = {}, response_status = {})
    if options.is_a?(String)
      super(options + '?containerPort=3000&languageName=rails5&locale=id', response_status)
    else
...
module ActionView::Helpers::AssetTagHelper
  def stylesheet_link_tag(*sources)
    ...
    sources.uniq.map { |source|
      ...
      uri.query = [uri.query, 'containerPort=3000', 'languageName=rails5', 'locale=id'].compact.join('&')
      tag_options = {
        "rel" => "stylesheet",
        "media" => "screen",
        "href" => uri,
